...
* `--stride-grad-processing`, `--mask-grad`, `--smooth-grad`, `--norm-grad`, `--grad-smooth-sigma`, `--grad-smooth-radius`, `--grad-norm-guess-change`, and `--grad-global-norm` to control Stride-like global gradient processing before each update
...
* The optimisation loop now includes a closer Stride-style `ProcessGlobalGradient` analogue (`mask_field -> smooth_field -> norm_field`) before each SGD/Adam update: Gaussian smoothing by default (`smooth_sigma=0.25`) plus Stride-like `norm_guess_change` model-dependent gradient scaling. Model clipping remains as the analogue of Stride's `ProcessModelIteration`.
