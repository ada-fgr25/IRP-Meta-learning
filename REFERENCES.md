...
* Gradient-mask ramp construction in JAX now follows Stride `mask_field` index/slice semantics more directly, and `grad_global_norm=True` now reuses one captured norm value across iterations to match Stride `NormField(global_norm=True)` behavior.
...
* The JAX trace pipeline now exposes an explicit active-step signature that includes optional Stride stages declared but missing in the tracked local bundle (`mute_first_arrival` and `time_tweaking`), treating them as explicit no-op stages to preserve version-aligned behavior.
* Multi-shot reduction semantics are now explicit in configuration (`shot_reduction={sum,mean}`), with `sum` retained as the Stride-parity default and `mean` available for controlled scaling ablations.
...
* The `sponge2` subdomain split now prefers the active runtime damping field support (`sponge_damp > 0`) when available, so boundary-stencil activation follows the same field that drives the damped update term.
...
* Phase-1 metrics now record JAX numerical runtime settings (`backend`, `jax_enable_x64`, and default matmul precision), and the driver exposes `--jax-enable-x64` for explicit parity/reproducibility control.
