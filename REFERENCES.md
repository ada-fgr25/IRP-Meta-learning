...
* The JAX trace pipeline now exposes an explicit active-step signature and explicit version-lock toggles for optional Stride stages not yet implemented in the differentiable path (`mute_first_arrival` and `time_tweaking`), preventing silent parity drift.
